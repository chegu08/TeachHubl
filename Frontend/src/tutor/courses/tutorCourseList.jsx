        ...
        <>
            {
                allCourses.filter((course) => selectedButton == 'all' || selectedButton == course.status).map((course, ind) => (
                    <div className='list' key={ind} role="button" onClick={()=>{navigation(`/tutorAboutClasspage?classId=${course.classId}&templateId=${course.templateId}`,{
                        state:{
                            studName:course.studentName
                        }
                    })}}>
